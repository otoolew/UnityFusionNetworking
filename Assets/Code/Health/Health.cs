...
namespace UnityFusionNetworking
{
    public class Health : ContextBehaviour, IHitTarget, IHitInstigator
    {
        ...
        [Networked, HideInInspector] public float CurrentHealth { get; private set; }
        ...
        [Networked, Capacity(8)] private NetworkArray<Hit> _hits { get; }

        [Networked] private TickTimer _immortalCooldown { get; set; }
        ...
        Transform IHitTarget.HitPosition => hitPosition != null ? hitPosition : transform;

        void IHitTarget.ProcessHit(ref HitData hitData)
        {
            ApplyHit(ref hitData);

            if (hitData.Amount == 0) return;
        ...
        private void ApplyHit(ref HitData hitData)
        {
            ...
            var hit = new Hit
            {
                ...
                RelativePosition =
                    hitData.Position != Vector3.zero ? hitData.Position - transform.position : Vector3.zero,
        ...
        {
            if (_visibleHitCount == _hitCount) return;
        ...
        public struct Hit : INetworkStruct
        {
            ...
            public float Damage;
            [Networked, Accuracy(0.01f)] public Vector3 RelativePosition { get; set; }
            [Networked, Accuracy(0.1f)] public Vector3 Direction { get; set; }
