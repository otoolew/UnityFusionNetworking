...
namespace UnityFusionNetworking
{
	...
	public class Level : SimulationBehaviour, ISpawned
	{
		...
		private Dictionary<NetworkPlayer, PlayerAgent> playerCharacterDictionary =
			new Dictionary<NetworkPlayer, PlayerAgent>();
		...
		public void SpawnPlayerCharacter(NetworkPlayer playerInfo)
		{
			...
			if (playerInfo.Object
			    .HasStateAuthority) // We have StateAuth over the player if we are the host or if we're the player self in shared mode
			{
				DebugLogMessage.Log(Color.green,
					$"Spawning Character for {playerInfo.DisplayName} with input auth {playerInfo.Object.InputAuthority}");
				...
				// Spawn Character
				/*PlayerCharacter character = Runner.Spawn(playerInfo.CharacterPrefab, t.position, t.rotation,
					playerInfo.Object.InputAuthority);
