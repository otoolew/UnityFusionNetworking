...
{
    public enum DrawIfHideType
    {
        /// <summary>Field will be disabled, but still visible.</summary>
        ReadOnly = 2,
        /// <summary>Field will be hidden.</summary>
        Hide = 3,
    }
    /// <summary>
    ...
    public class NetworkDebugStart : Behaviour
    {
        ...
        /// </summary>
        [InlineHelp] //[DrawIf(nameof(StartMode), (long)StartModes.UserInterface, DrawIfHideType.Hide)]
        ...
        /// </summary>
        [InlineHelp] //[DrawIf(nameof(ShowAutoClients), true, DrawIfHideType.Hide)]
        ...
        /// </summary>
        [InlineHelp] public ushort ServerPort = 27015;

        /// <summary>
        /// The default room name to use when connecting to photon cloud.
        /// </summary>
        [InlineHelp] public string DefaultRoomName = ""; // empty/null means Random Room Name

        /// <summary>
        /// Will automatically enable <see cref="FusionStats"/> once peers have finished connecting.
        /// </summary>
        [InlineHelp] public bool AlwaysShowStats = false;
        ...
        /// </summary>
        [InlineHelp] [ScenePath] [MultiPropertyDrawersFix]
        ...
        /// </summary>
        [InlineHelp] [SerializeField] [EditorDisabled] [MultiPropertyDrawersFix]
        ...
        protected bool ShowAutoClients =>
            StartMode != StartModes.Manual && UsingMultiPeerMode && AutoStartAs != GameMode.Single;
        ...
        protected virtual void Start()
        {
            ...
            if (StartMode == StartModes.Automatic)
            {
                ...
                {
                    StartCoroutine(StartWithClients(AutoStartAs, sceneRef,
                        isMultiPeer
                            ? AutoClients
                            : (AutoStartAs == GameMode.Client || AutoStartAs == GameMode.AutoHostOrClient ? 1 : 0)));
                }
            }
            else
            {
        ...
        protected IEnumerator StartWithClients(GameMode serverMode, SceneRef sceneRef, int clientCount)
        {
            ...
            if ((serverMode == GameMode.Shared || serverMode == GameMode.AutoHostOrClient) && clientCount > 1 &&
                config.PeerMode == NetworkProjectConfig.PeerModes.Multiple)
            {
                DefaultRoomName = string.IsNullOrEmpty(DefaultRoomName) == false
                    ? DefaultRoomName
        ...
        void DisplayAddToSceneButtonIfNeeded()
        {
            ...
            {
                GUILayout.Space(4);
                var clicked = BehaviourEditorUtils.DrawWarnButton(
                    new GUIContent("Add Scene To Settings", "Will add current scene to Unity Build Settings list."),
                    MessageType.Warning);
                if (clicked)
                {
