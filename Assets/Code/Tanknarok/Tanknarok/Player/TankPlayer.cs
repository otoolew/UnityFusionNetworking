...
namespace FusionExamples.Tanknarok
{
	...
	public class TankPlayer : NetworkBehaviour, ICanTakeDamage
	{
		...
		public static TankPlayer local { get; set; }
		...
		public void ApplyDamage(Vector3 impulse, byte damage, PlayerRef attacker)
		{
			...
			TankPlayer attackingPlayer = PlayerManager.Get(attacker);
			...
			if (damage >= life)
			{
				...
				if(GameManagerDeprecated.playState==GameManagerDeprecated.PlayState.LEVEL)
					lives -= 1;

				if (lives > 0)
					Respawn( _respawnTime );

				GameManagerDeprecated.instance.OnTankDeath();
			}
			else
			{
				life -= damage;
		...
		public static void OnStateChanged(Changed<TankPlayer> changed)
		{
