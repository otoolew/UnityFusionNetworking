	...
	public class LevelManager : NetworkSceneManagerBase
	{
		...
		protected override IEnumerator SwitchScene(SceneRef prevScene, SceneRef newScene, FinishedLoadingDelegate finished)
		{
			...
			if (Runner.IsServer || Runner.IsSharedModeMasterClient)
				GameManagerDeprecated.playState = GameManagerDeprecated.PlayState.TRANSITION;

			int winner = GameManagerDeprecated.WinningPlayerIndex;
	    ...
	    else
	    {
		    StartCoroutine(_countdownManager.Countdown(() =>
		    {
			    ...
			    {
				    GameManagerDeprecated.WinningPlayerIndex = -1;
